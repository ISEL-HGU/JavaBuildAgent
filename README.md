...
2.  **2차 방어선 - 동적 지능형 치유 (Smart Flexible Healing)**
    *   **역할**: 복잡하고 예측 불가능한 미지의 에러를 해결하는 전지전능한 해결사.
    *   **동작**: 1차 방어선이 뚫려 빌드가 실패할 경우 개입합니다. 초기에는 단순한 수정만 시도했으나, 최신 업데이트를 통해 **완전한 자율성**을 부여받았습니다.
    *   **효과**: 단일 통합 프롬프트(Smart Prompt)를 사용하여 문제의 원인을 심층 분석하고, 상황에 따라 **코드 수정, 의존성 업데이트, 주석 처리(Neutralize), 파일 생성** 중 최적의 전략을 스스로 선택합니다.
...
1.  **컨텍스트 인식 (Context Awareness)**
    ...
    - Docker 내부 경로(`/app/buggy/...`)와 실제 로컬 경로를 정확히 매핑하여 "File Not Found" 오류를 원천 차단했습니다.

2.  **지식 주입 (Knowledge Injection)**
    - "Bintray/JCenter Shutdown"과 같은 도메인 특화 지식을 프롬프트에 내장했습니다.
    - LLM이 죽은 리포지토리를 계속 시도하는 대신, **Maven Central로 교체**하거나 **삭제**하는 똑똑한 판단을 내립니다.

3.  **안전한 코드 생성 (Sandboxed Compatibility)**
    - 에이전트 실행 환경(Python 3.9 등)을 고려하여, 호환되지 않는 최신 문법(Python 3.10+ Type Hint 등) 사용을 억제합니다.
    - 이는 치유 스크립트 실행 중 발생할 수 있는 2차적인 런타임 오류를 방지합니다.
